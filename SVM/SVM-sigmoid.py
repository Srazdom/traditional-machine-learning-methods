...
toolbox.register("attr_c", random.uniform, 0.1, 10.0)  # Adjusted range for 'C'
...
scores = results['mean_test_score'].reshape(n_C, n_gamma * n_coef0)  # Adjust the shape based on the actual number of parameter combinations

plt.figure(figsize=(12, 6))
sns.heatmap(scores, annot=True, fmt=".3f", xticklabels=np.round(param_grid['coef0'], 2), yticklabels=np.round(param_grid['C'], 2))
plt.title('SVM Sigmoid Kernel Performance')
plt.xlabel('Coef0')
plt.ylabel('C')
plt.show()
