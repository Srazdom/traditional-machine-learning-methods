...
print(f"CART 分类准确率: {accuracy_cart:.2f}")
...
print(f"ID3 分类准确率: {accuracy_id3:.2f}")
...
print(f"自定义 C4.5 分类准确率: {accuracy_c45:.2f}")
...
print(f"CART 交叉验证准确率: {evaluate_model(clf_cart, X, y):.2f}")
print(f"ID3 交叉验证准确率: {evaluate_model(clf_id3, X, y):.2f}")
print(f"自定义 C4.5 交叉验证准确率: {evaluate_model(clf_c45, X, y):.2f}")
...
# 绘制最佳模型的决策边界
plot_decision_boundary(best_model, X, y, f"{best_model_name} 决策边界 (准确率: {best_accuracy:.2f})")
